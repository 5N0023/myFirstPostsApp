    ...
    return (
        <main>
            ...
            <div className="flex-col items-center justify-center py-2 w-full px-2">
                ...
                <div className="flex flex-col items-center justify-center py-2 w-full px-2">
                {posts.length !== 0 ? posts.map((post: any, key: number) => {
                    return (
                        <div className="flex flex-col items-center justify-center py-2 w-full px-2 border-2 border-black" key={key}>
                            <div className="flex flex-row items-center justify-center py-2 w-full px-2 border-2 border-blue-500 bg-slate-500">
                            <h1 className="text-black py-2">{post.username}</h1>
                            </div>
                            <h1 className="text-black py-2">{post.desc}</h1>
                            {post.img.length ? (
                                <img className="w-1/2 h-1/2" src={post.img} alt="post img" />
                            ) : null}
                        </div>
                    );
                }) : <h1>no posts</h1>}
